  ...
  return (
    <section id="pricing-strategy" className="scroll-mt-24 py-16">
      <div className="text-center mb-12">
        <h2 className="text-3xl font-bold text-slate-900 mb-4">Mastering Your Freelance Pricing</h2>
        <p className="text-slate-500 max-w-2xl mx-auto">Beyond the numbers: Use these strategies to maximize your income while keeping clients happy.</p>
      </div>
      
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-16">
        {strategies.map((strat, i) => (
          <div key={i} className="bg-white rounded-2xl border border-slate-200 p-8 shadow-sm hover:shadow-md transition-shadow flex flex-col">
            <div className="p-3 bg-slate-50 w-fit rounded-xl mb-6">
              {strat.icon}
            </div>
            <h3 className="text-xl font-bold text-slate-800 mb-2">{strat.title}</h3>
            <p className="text-sm text-slate-500 mb-6 flex-grow">{strat.description}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-amber-50 border border-amber-200 p-8 rounded-2xl">
          <h3 className="text-xl font-bold text-amber-900 mb-4 flex items-center gap-2">
            <ShieldAlert className="text-amber-600" /> The Tax Buffer
          </h3>
